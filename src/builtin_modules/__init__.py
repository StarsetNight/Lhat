...
class LoginApplication(QMainWindow):
    def __init__(self):
        ...
        _check_dir: tuple[str, ...] = ("logs/", "records/")
        for dir in _check_dir:
...
#     def accept(self):
#         webbrowser.open(
#             f"https://{self.ui.input_box_register_server_ip_port.toPlainText()}"
#         )  # 打开安全认证网页
#         return self.done(0)
...
class ChatApplication(QMainWindow):
    ...
    def band(self):
        """绑定信号和槽"""
        chat_window_signal.appendOutPutBox.connect(lambda msg:self.ui.output_box_message.append(msg))
        chat_window_signal.setOutPutBox.connect(lambda msg:self.ui.output_box_message.setText(msg))
        chat_window_signal.clearOutPutBox.connect(self.ui.output_box_message.clear)

        chat_window_signal.appendInPutBox.connect(lambda msg:self.ui.input_box_message.append(msg))
        chat_window_signal.setInPutBox.connect(lambda msg:self.ui.input_box_message.setText(msg))
        chat_window_signal.clearInPutBox.connect(self.ui.input_box_message.clear)

        chat_window_signal.appendOnlineUserList.connect(lambda msg:self.ui.output_box_online_user.append(msg))
        chat_window_signal.setOnlineUserList.connect(lambda msg:self.ui.output_box_online_user.setText(msg))
        chat_window_signal.clearOnlineUserList.connect(self.ui.output_box_online_user.clear)
