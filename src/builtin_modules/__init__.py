...
class ChatApplication(QMainWindow):
    ...
    def send(self, raw_message: str):
        ...
        if not raw_message.strip():  # 如果消息为空，则不发送，strip的作用是去掉首尾空格
            ...
            if sys.getsizeof(raw_message) <= 968:  # 经过计算，1024个字节的消息以968个字节为正文差不多可以。
                ...
                self.chat_window_signal.appendOutPutBox.emit('[提示] 发送的私聊消息长度不能大于968字节！<br/>'
    ...
    def receive(self):
        ...
        if os.path.exists(f'records/chat_{self.server_address}.txt'):
            ...
            self.chat_window_signal.appendOutPutBox.emit('哒哒！欢迎来到Lhat聊天室！大家开始聊天吧！<br/>'
        ...
        while True:
            ...
            if message_type == 'TEXT_MESSAGE' or message_type == 'COLOR_MESSAGE':  # 如果是文本消息
                ...
                self.chat_window_signal.appendOnlineUserList.emit('<font color="#3333FF">====在线用户====</font>')
                for user_index, online_username in enumerate(online_users):
                    # online_username是用于显示在线用户的，不要与username混淆
                    self.chat_window_signal.appendOnlineUserList.emit(str(online_username))
                # received_long_data = ''  # 正常解包之后，清空长消息
                ...
                if message[1]:
                    ...
                    for manager_index, manager_username in enumerate(message[1]):
                        self.chat_window_signal.appendOutPutBox.emit(f'{manager_index + 1} {manager_username}<br/>')
                else:
